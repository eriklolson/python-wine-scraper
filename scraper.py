...
for page in pages:
    ...
    for container in products:
        ...
        brand = subsoup.select_one("span[itemprop=brand]").text if subsoup.select_one("span[itemprop=brand]") else 'Null'
        brands.append([brand])
        ...
        description = subsoup.find('div', class_='shortdescription sdcustom').text.replace('\"', '') if subsoup.find('div', class_='shortdescription sdcustom') else 'Null'
        descriptions.append([description])

        product_name = container.find('a', class_='product-item-link').text.strip() if container.find('a', class_='product-item-link') else 'Null'
        product_names.append([product_name])

        year = container.find('p', class_='productsubtitle').text.split('/', 1)[0].strip().replace('\"', '') if container.find('p', class_='productsubtitle') else 'Null'
        year = re.findall(r"\d+", year) if re.findall(r"\d+", year) else 'Null'
        years.append(year)

        volume = container.find('p', class_='productsubtitle').text.replace('\"', '').replace(' ml', 'ml').strip() if container.find('p', class_='productsubtitle') else 'Null'
        volume = re.findall(r"\/(.*?)\|", volume)
        volumes.append(volume)

        primary_grape = container.find('div', class_='winegrapes wmarker').text.split('|', 1)[0].strip() if container.find('div', class_='winegrapes wmarker') else 'Null'
        primary_grape = re.findall(r"(?<=: )[^\]]+", primary_grape)
        primary_grapes.append(primary_grape)

        all_grape = container.find('div', class_='winegrapes wmarker').text.replace('\"', '').strip() if container.find('div', class_='winegrapes wmarker') else 'Null'
        all_grape = re.split(r"\:", all_grape, 2)[-1] if re.split(r"\:", all_grape, 2)[-1] else 'Null'
        all_grapes.append([all_grape])

        country = container.find('div', class_='productlocation wmarker').text.split('|', 1)[0].strip() if container.find('div', class_='productlocation wmarker') else 'Null'
        country = re.findall(r"([^\,\|]+)", country) if re.findall(r"([^\,\|]+)", country) else 'Null'
        countries.append(country)

        region = container.find('div', class_='productlocation wmarker').text.replace('\"', '').strip() if container.find('div', class_='productlocation wmarker') else 'Null'
...
wine_data['product_name'] = wine_data['product_name'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['year'] = wine_data['year'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['volume'] = wine_data['volume'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['proofs'] = wine_data['proofs'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['brand'] = wine_data['brand'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['country'] = wine_data['country'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\')", lambda m: '' if m.group(1) else np.nan)
wine_data['region'] = wine_data['region'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['color'] = wine_data['color'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['primary_grape'] = wine_data['primary_grape'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['all_grape'] = wine_data['all_grape'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['price'] = wine_data['price'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['image'] = wine_data['image'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['description'] = wine_data['description'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
wine_data['product_link'] = wine_data['product_link'].astype(str).str.replace(r"^[][\s]*$|(^'+|'+$|\'|\")", lambda m: '' if m.group(1) else np.nan)
